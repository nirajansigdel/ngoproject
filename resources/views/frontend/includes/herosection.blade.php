<style>
  ...
  #photoGalleryCarousel.carousel-fade .carousel-item {
    transition-duration: 3s; /* increase fade duration to 1 second */
  }
...
<div class="container-fluid container">
    <div class="row gap-5 my-5 ml-5">
        <div class="col-md-6 d-flex flex-column gap-2">
            <div class="hero-text">
                <span class="text-gray">Welcome to the</span>
                <span class="text-green">Umbrella Organization</span>
                <span class="text-gray">Nepal for movement</span>

            </div>
            <p class="xs-text">
                Every workout helps relieve the impact of trafficking, poverty, and war on children in Nepal. Your
                effort supports projects focused on education, reintegration, and community development — empowering a
                new generation with hope and opportunity.
            </p>
        </div>
    

        <div id="photoGalleryCarousel" class="carousel slide carousel-fade col-md-5 d-none d-md-block rounded-b-full" data-bs-ride="carousel" data-bs-interval="10000" data-bs-pause="false">
    <div class="carousel-inner">
        @php $first = true; @endphp
        @foreach($gallery->sortByDesc('updated_at') as $image)
            @if(!empty($image->img) && is_array($image->img))
                @foreach($image->img as $imgPath)
                    <div class="carousel-item {{ $first ? 'active' : '' }}">
                        <img src="{{ asset($imgPath) }}" alt="{{ $image->title }}" class="indeximage rounded d-block w-100">
                    </div>
